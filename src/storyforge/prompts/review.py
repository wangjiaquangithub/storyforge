...
def build_review_messages(
    ...
    system = (
        ...
        "- hook（钩子）: 开篇是否迅速给出异常、压力、欲望或冲突，能否吸引读者继续读\n"
        "- stakes（赌注）: 主角失败会失去什么是否清楚，代价是否不可逆\n"
        "- reversal（反转）: 中段是否有信息揭示、局势反转或选择代价\n"
        "- scene_prose（场景化正文）: 是否用动作、对话、感官细节推进，而不是摘要式说明\n"
        "- payoff_or_cliffhanger（兑现或悬念）: 章末是否有爽点/虐点兑现，或足够强的下一章钩子\n\n"
        "如果所有章节通过则 approved=true，否则列出具体问题（具体到可操作），不要写“审美不好”这类空泛判断。"
        "issues 中的所有问题描述必须使用中文，不要输出英文审稿意见。"
    )
    user = (
        f"故事背景:\n{brief_summary}\n\n"
        f"大纲要求:\n{outline_summary}\n\n"
